...
public class ClaudeController {
    ...
    private static final String CHAT_SYSTEM_PROMPT =
        ...
        "- Для маркированных списков используй ТОЛЬКО дефис с пробелом \"- \" в начале строки " +
        "(например: \"- Пункт списка\"). НИКОГДА не используй звёздочку \"*\" как маркер списка " +
        "(ни \"* текст\", ни \"*текст\" без пробела) — фронтенд-парсер это не распознаёт и " +
        "показывает звёздочку буквально, ломая вид ответа.\n" +
        "- Выделяй ключевые мысли ТОЛЬКО двойной звёздочкой \"**жирный текст**\" (ровно две звёздочки " +
        "с каждой стороны, без пробела внутри после/перед звёздочками). НИКОГДА не используй " +
        "одиночную звёздочку \"*текст*\" для выделения — она не распознаётся и остаётся как есть.\n" +
        "- Используй нумерованные списки (\"1. \", \"2. \" и т.д.) там, где важен порядок\n" +
        ...
        "ФОРМАТ КОЛОНКИ \"День\" В ТАКОЙ ТАБЛИЦЕ: указывай ТОЛЬКО номер дня плана (например, " +
        "\"1\" или \"4–6\" для диапазона дней) — БЕЗ календарной даты в скобках рядом с ним " +
        "(НЕ пиши \"1–3 (4–6 августа)\"). Конкретные даты в каждой строке таблицы избыточны и " +
        "путают пользователя (неочевидно, число это дня плана или дата). Вместо этого СРАЗУ ПОСЛЕ " +
        "таблицы одной строкой укажи период плана целиком в реальных датах, отталкиваясь от даты " +
        "старта, которую пользователь выбрал в ответ на вопрос \"Когда вы готовы начать?\" (см. правило " +
        "про дату старта плана ниже) — например: \"Этот план рассчитан на период с 5 по 11 августа 2026\". " +
        ...
        "Если это цель/план с достижением результата за срок (фитнес, обучение, привычка, подготовка " +
        "к экзамену и т.п.) и это ПЕРВОЕ сообщение по этой цели в чате (ты ещё не спрашивал уточнений " +
        "и не показывал таблицу плана) — САМ РЕШИ, какие уточняющие вопросы реально нужны именно для " +
        "ЭТОЙ цели, чтобы построить полезный план, а не общий шаблон. Например: для «подготовиться к " +
        "экзамену за 10 дней» сначала важно узнать предмет/темы экзамена, а уже потом — сколько времени " +
        "в день человек готов уделять; для «набрать мышечную массу» может быть важен уровень подготовки " +
        "и доступ к залу; для «выучить язык» — текущий уровень и цель (разговорный/для работы и т.п.). " +
        "Выбирай вопросы и их порядок исходя из того, что реально повлияет на качество плана — от общего " +
        "к частному (сначала то, что определяет СОДЕРЖАНИЕ плана, и только потом организационные детали " +
        "вроде времени в день). Не больше 2–3 содержательных вопросов — не превращай это в анкету.\n" +
        "ОБЯЗАТЕЛЬНЫЙ ПОСЛЕДНИЙ ВОПРОС — ДАТА СТАРТА: в этот же раунд уточнений (тот же массив " +
        "\"questions\" в блоке <<<CLARIFY>>>) ПОСЛЕДНИМ пунктом всегда добавляй вопрос с ОБЯЗАТЕЛЬНО " +
        "именно таким \"id\": \"start_date\" (фронтенд определяет этот вопрос строго по id, а не по " +
        "тексту, поэтому id должен быть указан буква в букву именно так, id вопроса менять нельзя): " +
        "\"Когда вы готовы начать?\" с вариантами ровно [\"Сегодня\", \"Завтра\"] (буква в букву, не " +
        "переформулируй сами варианты ответа — фронтенд тоже ищет их по точному тексту). Сам текст " +
        "вопроса можешь адаптировать под контекст цели (например, добавить \"изучение Python\"), а вот " +
        "id \"start_date\" и варианты [\"Сегодня\", \"Завтра\"] должны остаться неизменными. Все вопросы " +
        "раунда (содержательные + этот) задаются ОДНИМ блоком <<<CLARIFY>>> сразу, а не по одному за " +
        "раз — весь раунд уточнений происходит за один обмен репликами. Только после ответа на все " +
        "вопросы (или после «Пропустить», что для start_date означает «сегодня» по умолчанию, а для " +
        "остальных — широкий/усреднённый вариант) показывай таблицу плана, считая днём 1 выбранную " +
        "дату старта. Если пользователь уже отвечал на уточнения по этой цели ранее в этом чате — не " +
        "спрашивай снова, сразу используй уже данные ответы.\n" +
        ...
        "актуальные данные), а не переспрашивай снова. ИСКЛЮЧЕНИЕ ДЛЯ ПЛАНОВ С ДАТОЙ СТАРТА: если " +
        "пропущенный раунд включал вопрос с id=\"start_date\" в числе прочих — для ЭТОГО конкретного " +
        "вопроса пропуск означает выбор \"Сегодня\" по умолчанию (без всякой подборки вариантов). " +
        "Остальные (содержательные) вопросы того же раунда при пропуске обрабатывай как обычно — " +
        "не сужай их до одного варианта, а бери усреднённый/широкий охват, как описано выше. Сразу " +
        "показывай таблицу плана с учётом этих значений по умолчанию.\n" +
    ...
    private static final String FILE_SYSTEM_PROMPT =
        ...
        "- Для маркированных списков используй ТОЛЬКО дефис с пробелом \"- \" в начале строки. " +
        "НИКОГДА не используй звёздочку \"*\" как маркер списка (ни \"* текст\", ни \"*текст\" " +
        "без пробела) — фронтенд-парсер это не распознаёт и показывает звёздочку буквально, " +
        "ломая вид ответа.\n" +
        "- Выделяй ключевые мысли ТОЛЬКО двойной звёздочкой \"**жирный текст**\" (ровно две " +
        "звёздочки с каждой стороны, без пробела внутри). НИКОГДА не используй одиночную " +
        "звёздочку \"*текст*\" для выделения — она не распознаётся и остаётся как есть.\n\n" +
        ...
        "действие по умолчанию для любого расплывчатого запроса о файле. Например, на запрос " +
        "\"прочти файл\" без дополнительных уточнений — не спрашивай, что именно интересует, " +
        "а сразу перескажи суть содержимого.\n\n" +
        "ЯЗЫК ОТВЕТА: следуй %LANGUAGE_RULE% строго — отвечай на языке диалога с пользователем, " +
        "ДАЖЕ ЕСЛИ содержимое самого файла написано на другом языке (например, файл на английском, " +
        "а пользователь пишет по-русски — отвечай по-русски, не переключайся на язык файла). " +
        "Ты можешь цитировать оригинальный текст файла на его языке при необходимости, " +
        "но твои собственные пояснения, выводы и summary — всегда на языке диалога.";
    ...
    // Регекс намеренно ограничен строками таблицы (начинаются с "|") и первой
    // ячейкой в них — дата в обычном тексте (например, итоговая строка
    // периода под таблицей) не трогается. Подстраховка на случай, если
    // модель проигнорирует текстовую инструкцию промпта про формат колонки
    // "День" (см. вызов в wrapChatReply).
    private static final java.util.regex.Pattern DAY_COLUMN_DATE_PATTERN = java.util.regex.Pattern.compile(
        "(?mi)^\\|(\\s*[^(|]*?)\\s*\\([^)]*(?:январ|феврал|март|апрел|ма[яй]|июн[ья]?|июл[ья]?|август|сентябр|октябр|ноябр|декабр)[^)]*\\)(\\s*\\|)"
    );
    ...
    // Убирает блок(и) <think>...</think> из ответа модели — это внутреннее
    // "рассуждение вслух", которое некоторые модели (например, vision-модели
    // через Groq/Yandex) включают прямо в content, пользователь не должен
    // его видеть. РАНЬШЕ было \"(?s)<think>.*?</think>\" — требовало ОБЯЗАТЕЛЬНО
    // закрывающий тег, и если генерация обрывалась по лимиту токенов ПОСЛЕ
    // <think> но ДО </think> (реально наблюдалось на практике), регекс вообще
    // не срабатывал — пользователю целиком утекал сырой текст рассуждений.
    // Теперь: сначала убираем полностью закрытые блоки, затем, если после
    // этого остался незакрытый "<think>" без пары — обрезаем от него и до
    // конца строки (в нём по определению нет ничего, кроме черновика мыслей).
    ...
    private boolean isMalformedClarifyBlock(String rawReply) {
        String cleaned = stripThinkTags(rawReply);
    ...
    private String wrapChatReply(String rawReply) {
        String reply = stripThinkTags(rawReply);
        if (reply.isEmpty()) {
            // Весь ответ был "съеден" незакрытым <think> (генерация оборвалась
            // по лимиту токенов, не успев дойти до реального ответа) — честно
            // сообщаем об этом вместо пустого пузыря, пользователь может
            // перегенерировать через кнопку 🔄.
            try {
                ObjectNode wrapped = mapper.createObjectNode();
                wrapped.put("type", "answer");
                wrapped.put("text", "Модель не успела закончить ответ (превышен лимит токенов на рассуждение). Попробуйте перегенерировать ответ.");
                return mapper.writeValueAsString(wrapped);
            } catch (Exception e) {
                return "{\"type\":\"answer\",\"text\":\"Модель не успела закончить ответ. Попробуйте перегенерировать.\"}";
            }
        }
        // Подстраховка на случай, если модель проигнорирует текстовую инструкцию
        // промпта про формат колонки "День" (реально наблюдалось — локальная
        // модель через Ollama не всегда точно следует форматным требованиям).
        // Программно вырезаем дату в скобках именно из первой колонки таблицы
        // markdown, не трогая даты в обычном тексте (например, итоговую строку
        // "план рассчитан на период с ... по ...").
        reply = stripDateFromDayColumn(reply);
        try {
    ...
    private String callOllama(
        ...
        body.put("stream", false);
        // ЯВНО задаём max_tokens вместо дефолта провайдера: модель для vision
        // (qwen3.6-27b) — reasoning-модель, она сначала подробно "думает
        // вслух" в <think>...</think>, и только потом даёт финальный ответ.
        // Без явного лимита генерация может обрываться прямо ПОСЕРЕДИНЕ
        // блока <think>, не успев дойти до реального ответа — реально
        // наблюдалось на практике (см. stripThinkTags и её комментарий).
        // 4096 с запасом покрывает и длинное рассуждение, и сам ответ.
