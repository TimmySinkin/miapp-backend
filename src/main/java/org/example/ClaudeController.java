...
public class ClaudeController {
    ...
    private static final String CHAT_SYSTEM_PROMPT =
        ...
        "КРОМЕ ровно одного случая — см. правило про дату старта плана ниже).\n" +
        "ПРАВИЛО ПРО ДАТУ СТАРТА ПЛАНА: если пользователь просит план/цель с регулярными действиями " +
        "за срок (фитнес, обучение, привычка и т.п.) и это ПЕРВОЕ сообщение по этой цели в чате (ты " +
        "ещё не спрашивал про дату старта и не показывал таблицу плана) — ПЕРЕД тем как показать " +
        "таблицу, задай РОВНО ОДИН уточняющий вопрос в формате <<<CLARIFY>>> с ОБЯЗАТЕЛЬНО именно " +
        "таким \"id\": \"start_date\" (фронтенд определяет этот вопрос строго по id, а не по тексту, " +
        "поэтому id должен быть указан буква в букву именно так, id вопроса менять нельзя): " +
        "\"Когда вы готовы начать?\" с вариантами ровно [\"Сегодня\", \"Завтра\"] (буква в букву, " +
        "не переформулируй сами варианты ответа — фронтенд тоже ищет их по точному тексту). Сам текст " +
        "вопроса можешь адаптировать под контекст цели (например, добавить \"изучение Python\"), а вот " +
        "id \"start_date\" и варианты [\"Сегодня\", \"Завтра\"] должны остаться неизменными. Только после " +
        "ответа на этот вопрос (или после «Пропустить», что означает «сегодня» по умолчанию) показывай " +
        "таблицу плана, считая днём 1 выбранную дату старта. Если пользователь уже ответил на этот " +
        "вопрос ранее в этом чате — не спрашивай снова, сразу используй уже данный ответ.\n" +
        ...
        "актуальные данные), а не переспрашивай снова. ИСКЛЮЧЕНИЕ: если пропущенный вопрос был именно " +
        "\"Когда вы готовы начать?\" (дата старта плана) — пропуск означает выбор \"Сегодня\" по " +
        "умолчанию, без всякой подборки вариантов, и сразу показывай таблицу плана с датами от сегодня.\n" +
